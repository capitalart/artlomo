...
from ..catalog.loader import load_physical_bases, load_physical_catalog
from ..catalog.validation import load_and_validate_bases, load_and_validate_catalog
from ..errors import ValidationError
from ..catalog.models import MockupBase, Template
from ..config import (
    BASE_DIR,
...
from .models import CatalogView, PolicyDocument, TemplateRecord
...
from ..services.MockupCoordinateService import (
    MockupCoordinateService,
    MockupCoordinateServiceError,
)
...
_SANITIZE_SYNC_STATUS_PATH = Path(__file__).resolve().parent / "ui" / "static" / "temp" / "sanitize_sync_status.json"
_SANITIZE_SYNC_MUTEX = threading.Lock()
_SANITIZE_SYNC_THREAD: threading.Thread | None = None
...
def _write_sanitize_sync_status(payload: dict) -> None:
    ensure_dir(_SANITIZE_SYNC_STATUS_PATH.parent)
    write_json_atomic(_SANITIZE_SYNC_STATUS_PATH, payload)


def _read_sanitize_sync_status() -> dict:
    if not _SANITIZE_SYNC_STATUS_PATH.exists():
        return {
            "status": "idle",
            "message": "No sanitize/sync task running.",
            "updated_at": _now_iso(),
        }
    try:
        raw = json.loads(_SANITIZE_SYNC_STATUS_PATH.read_text(encoding="utf-8"))
        if not isinstance(raw, dict):
            return {
                "status": "idle",
                "message": "No sanitize/sync task running.",
        ...
        normalized = dict(raw)
        normalized["status"] = str(raw.get("status", "idle")).strip().lower()
        normalized["updated_at"] = str(raw.get("updated_at") or _now_iso())
        normalized["message"] = str(raw.get("message") or "")
        return normalized
    except Exception:
        return {
            "status": "failed",
            "message": "Unable to read sanitize/sync status.",
            "updated_at": _now_iso(),
        }
...
def _render_thumb_500_jpg(*, base_png: Path, thumb_out: Path) -> None:
    ...
    # Fallback path: produce a deterministic 500x500 JPG thumbnail with Pillow
    # so generation does not fail when Node/xvfb is temporarily unavailable.
    try:
        with Image.open(base_png) as src:
            rgb = src.convert("RGB")
            contained = ImageOps.contain(rgb, (500, 500), method=Image.Resampling.LANCZOS)
            canvas = Image.new("RGB", (500, 500), (245, 245, 245))
            offset = ((500 - contained.width) // 2, (500 - contained.height) // 2)
            canvas.paste(contained, offset)
            canvas.save(thumb_out, format="JPEG", quality=92, optimize=True)
    except Exception as exc:
        raise ValidationError(f"Unable to generate thumbnail for {base_png.name}") from exc
...
class CatalogAdminService:
    ...
    def sync_orphan_bases_from_filesystem(self) -> Dict[str, int]:
        doc = self._load_catalog_json()
        bases = doc.get("bases", [])
        if not isinstance(bases, list):
            raise ValidationError("Catalog document must contain bases list")

        discovered_orphans = 0
        known_base_rel_paths: set[str] = set()
        for entry in bases:
            if not isinstance(entry, dict):
                continue
            base_image_val = entry.get("base_image")
            if isinstance(base_image_val, str) and base_image_val.strip():
                known_base_rel_paths.add(base_image_val.strip())

        if self.base_root.exists() and self.base_root.is_dir():
            now_discovered = _now_iso()
            for base_png in self.base_root.glob("*/*/*.png"):
                try:
                    rel_base = _relative_to(self.catalog_dir, base_png).as_posix()
                except Exception:
                    continue
                if rel_base in known_base_rel_paths:
                    continue

                aspect = _normalize_aspect(base_png.parent.parent.name)
                category_folder = _normalize_category(base_png.parent.name)
                slug_val = base_png.stem
                coords_path = base_png.with_suffix(".json")
                ...
                if not coords_path.exists():
                    ...
                    try:
                        ...
                        if isinstance(coords_payload, dict):
                            coords_payload["template"] = f"{slug_val}.png"
                            coords_payload, _coordinate_type = _normalize_coords_payload_v2(
                                coords_payload,
                                template=f"{slug_val}.png",
                            )
                            png_size = validate_png_bytes(base_png.read_bytes()).size
                            coords_payload = _clamp_coords_payload(coords_payload, size=png_size)
                            placements = validate_coords_payload(coords_payload, png_size)
                            region_count = len(placements)
                            status = "coordinates_ready" if region_count > 0 else "missing_coordinates"
                            write_json_atomic(coords_path, coords_payload)
                    except Exception:
                        status = "needs_regeneration"

                orphan_entry: Dict[str, object] = {
                    "id": uuid.uuid4().hex,
                    "slug": slug_val,
                    "original_filename": base_png.name,
                    "base_image": rel_base,
                    "category": category_folder,
                    "aspect_ratio": aspect,
                    "aspect_source": ASPECT_SOURCE_UPLOAD,
                    "status": status,
                    "coordinates_path": _relative_to(self.catalog_dir, coords_path).as_posix(),
                    "region_count": int(region_count),
                    "last_coordinated_at": now_discovered,
                    "coordinate_type": COORDINATE_TYPE_LEGACY,
                    "created_at": now_discovered,
                    "updated_at": now_discovered,
                }
                bases.append(orphan_entry)
                known_base_rel_paths.add(rel_base)
                discovered_orphans += 1

        if discovered_orphans:
            self._write_catalog_json(doc)

        return {"discovered_orphans": int(discovered_orphans)}

    def upsert_generated_base(
        self,
        *,
        job_id: str,
        aspect_ratio: str,
        ...
        source_coordinates_path: str,
    ) -> MockupBase:
        generated_path = Path(generated_image_path).resolve()
        coords_source_path = Path(source_coordinates_path).resolve()

        if not generated_path.exists():
            raise ValidationError(f"Generated image missing for job {job_id}: {generated_path}")
        if not coords_source_path.exists():
            raise ValidationError(f"Generated coordinates missing for job {job_id}: {coords_source_path}")

        aspect_clean = _normalize_aspect(aspect_ratio)
        category_slug = _normalize_category(category)
        slug = slugify(f"{aspect_clean}-{category_slug}-mu-{variation_index}")
        base_abs, coords_abs = self._desired_base_paths(
            slug=slug,
            aspect=aspect_clean,
        ...
        try:
            ...
            raise ValidationError(f"Generated image is not a valid PNG for job {job_id}") from exc

        if image_obj.format != "PNG":
            raise ValidationError(f"Generated image must be PNG for job {job_id}")

        try:
            source_coords_payload = json.loads(coords_source_path.read_text(encoding="utf-8"))
        except Exception as exc:
            raise ValidationError(f"Generated coordinates JSON is invalid for job {job_id}") from exc

        raw_points = source_coords_payload.get("points")
        if not isinstance(raw_points, list) or len(raw_points) != 4:
            raise ValidationError(f"Generated coordinates for job {job_id} must contain exactly four points")

        ordered_points = _order_points_tl_tr_br_bl(raw_points)
        coords_payload = {
            "template": f"{slug}.png",
            "format_version": COORDINATE_FORMAT_VERSION,
            "zones": [
                {
                    "points": _apply_bleed_to_points(ordered_points),
                }
            ],
        }
        coords_payload = _clamp_coords_payload(coords_payload, size=image_obj.size)
        placements = validate_coords_payload(coords_payload, image_obj.size)

        ensure_dir(base_abs.parent)
        base_abs.write_bytes(image_bytes)
        write_json_atomic(coords_abs, coords_payload)

        id_num = _extract_numeric_id(slug)
        if id_num is None:
            rest = slug
        ...
        # Self-heal historical duplicate slugs so one bad entry cannot block all
        # future generated base registrations.
        deduped_bases: List[Dict[str, object]] = []
        seen_slug_index: Dict[str, int] = {}
        for entry in doc["bases"]:
            if not isinstance(entry, dict):
                continue
            slug_value = str(entry.get("slug") or "").strip()
            if not slug_value:
                deduped_bases.append(cast(Dict[str, object], entry))
                continue

            existing_index = seen_slug_index.get(slug_value)
            if existing_index is None:
                seen_slug_index[slug_value] = len(deduped_bases)
                deduped_bases.append(cast(Dict[str, object], entry))
                continue

            # If this duplicate belongs to the current job/slug, prefer it.
            if (
                slug_value == slug
                and str(entry.get("source_job_id") or "") == job_id
            ):
                deduped_bases[existing_index] = cast(Dict[str, object], entry)

        if len(deduped_bases) != len(doc["bases"]):
            logger.warning(
                "Deduplicated base catalog slugs during generated upsert",
                extra={
                    "removed_count": len(doc["bases"]) - len(deduped_bases),
                    "job_id": job_id,
                },
            )
            doc["bases"] = deduped_bases

        existing_entry: Dict[str, object] | None = None
        for entry in doc["bases"]:
            if not isinstance(entry, dict):
                continue
            if entry.get("source_job_id") == job_id or entry.get("slug") == slug:
                existing_entry = cast(Dict[str, object], entry)
                break
        ...
        if existing_entry is None:
            ...
            doc["bases"].append(existing_entry)
        ...
        existing_entry.update(
            {
                ...
                "coordinate_type": COORDINATE_TYPE_PERSPECTIVE,
                "updated_at": now,
            }
        )

        self._write_catalog_json(doc)
        return self._base_by_id(str(existing_entry["id"]))
    ...
    def sanitize_and_sync_bases(self) -> Dict[str, int]:
        ...
        discovered_orphans = 0
        try:
            ingest_summary = self.sync_orphan_bases_from_filesystem()
            discovered_orphans = int(ingest_summary.get("discovered_orphans", 0) or 0)
            if discovered_orphans:
                doc = self._load_catalog_json()
                bases = doc.get("bases", [])
                if not isinstance(bases, list):
        ...
        return {
            ...
            "discovered_orphans": int(discovered_orphans),
        }

    def get_sanitize_sync_status(self) -> dict:
        return _read_sanitize_sync_status()

    def start_sanitize_sync_async(self) -> dict:
        global _SANITIZE_SYNC_THREAD
        with _SANITIZE_SYNC_MUTEX:
            current = _read_sanitize_sync_status()
            running = str(current.get("status", "")).lower() in {"queued", "running"}
            if _SANITIZE_SYNC_THREAD and _SANITIZE_SYNC_THREAD.is_alive():
                running = True
            if running:
                raise ValidationError("A sanitize/sync task is already running")

            run_id = str(uuid.uuid4())
            _write_sanitize_sync_status(
                {
                    "run_id": run_id,
                    "status": "queued",
                    "message": "Sanitize & Sync queued.",
                    "updated_at": _now_iso(),
                }
            )

            thread = threading.Thread(
                target=self._run_sanitize_sync_async,
                args=(run_id,),
                daemon=True,
                name=f"mockup-sanitize-sync-{run_id[:8]}",
            )
            _SANITIZE_SYNC_THREAD = thread
            thread.start()

        return _read_sanitize_sync_status()

    def _run_sanitize_sync_async(self, run_id: str) -> None:
        global _SANITIZE_SYNC_THREAD
        _write_sanitize_sync_status(
            {
                "run_id": run_id,
                "status": "running",
                "message": "Running Sanitize & Sync...",
                "updated_at": _now_iso(),
            }
        )
        try:
            summary = self.sanitize_and_sync_bases()
            _write_sanitize_sync_status(
                {
                    "run_id": run_id,
                    "status": "completed",
                    "message": "Sanitize & Sync completed.",
                    "summary": summary,
                    "updated_at": _now_iso(),
                }
            )
    ...
    def generate_coordinates_for_bases(
        ...
        *,
        ...
        for bid in ids:
            ...
            # Load image — support both alpha-channel (legacy) and flat PNG (Gemini-generated)
            # bases.  validate_png_bytes() enforces an alpha channel which flat Gemini PNGs
            # don't have, so we use PIL directly here and only require a valid PNG format.
            try:
                _raw = base_path.read_bytes()
                _pil = Image.open(BytesIO(_raw))
                _pil.load()
                if _pil.format != "PNG":
                    raise ValidationError(f"Base image must be PNG for id {bid}: {base_path}")
                img = _pil
            except ValidationError:
                raise
            except Exception as _exc:
                raise ValidationError(f"Base image is not a valid PNG for id {bid}: {base_path}") from _exc
            ...
            if coords_payload is None:
                if legacy:
                    ...
                    # ---- Cyan detection (Gemini-generated flat PNGs) ----
                    # Try MockupCoordinateService first — it handles both cyan rectangles
                    # and transparent alpha cutouts, so this covers all base formats.
                    try:
                        cyan_corners = MockupCoordinateService.extract_coordinates(str(base_path))
                        coords_payload = {
                            "template": f"{entry.get('slug')}.png",
                            "corners": cyan_corners,
                        }
                        generated_from_detection = True
                    except MockupCoordinateServiceError:
                        # Fall through to the node-based alpha scanner for image types
                        # that MockupCoordinateService can't handle.
                        pass
    ...
    def set_base_live_state(self, base_id: str, *, live: bool) -> MockupBase:
        base_id = base_id.strip()
        if not base_id:
            raise ValidationError("Base id is required")
        ...
        if live:
            coords_path = self._resolve_coords_path(entry)
            if not coords_path.exists():
                raise ValidationError("Cannot publish to live until coordinates are generated")

            if current_status in {"missing_coordinates", "needs_regeneration", "uploaded", "invalid"}:
                raise ValidationError(
                    "Cannot publish to live while base status is unresolved. "
            ...
            if current_status == "in_use":
                raise ValidationError("Cannot move a live in-use base back to staging")
