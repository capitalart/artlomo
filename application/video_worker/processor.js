...
async function scanTransparentZones(payload) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      // Track corner candidates using sum/diff method (for perspective-skewed regions)
      let tlPoint = null; // minimize (x + y)
      let trPoint = null; // maximize (x - y)
      let brPoint = null; // maximize (x + y)
      let blPoint = null; // minimize (x - y)
      ...
      while (qHead < qTail) {
        ...
        // Track corners based on geometric properties
        const s = cx + cy; // sum: tl and br have extreme values
        const d = cx - cy; // diff: tr and bl have extreme values
        
        if (s < minS) { minS = s; tlPoint = { x: cx, y: cy }; }
        if (s > maxS) { maxS = s; brPoint = { x: cx, y: cy }; }
        if (d > maxD) { maxD = d; trPoint = { x: cx, y: cy }; }
        if (d < minD) { minD = d; blPoint = { x: cx, y: cy }; }
        ...
        for (let ny = Math.max(0, cy - 1); ny <= Math.min(height - 1, cy + 1); ny += 1) {
          for (let nx = Math.max(0, cx - 1); nx <= Math.min(width - 1, cx + 1); nx += 1) {
            ...
            if (visited[ni] === 1 || !isTransparent(ni)) continue;
            visited[ni] = 1;
            queue[qTail++] = ni;
          }
        }
      ...
      // Use calculated corner points if available, fallback to axis-aligned
      const corners = [
      ...
      candidates.push({
        ...
        zone: {
          points: corners.map(p => normalizePoint(p.x, p.y, width, height)),
        },
      });
    }
  }
