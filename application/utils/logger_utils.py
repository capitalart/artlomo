...
of the application based on the central logging config.
...
def setup_logger(logger_name: str, log_key: str, level: int = logging.INFO) -> logging.Logger:
    ...
    Uses LOG_CONFIG provided by application.config to determine the subfolder and filename format.

    Args:
        logger_name: The name of the logger (e.g., __name__).
        log_key: The key within config.LOG_CONFIG (e.g., "ANALYZE_OPENAI").
    ...
    # Get folder and filename details from the shared config
    ...
    # Create and configure rotating file handler to avoid unbounded growth
...
def security_audit_logger(level: int = logging.INFO) -> logging.Logger:
    ...
    log_filepath = config.LOGS_DIR / "security.log"
    handler = RotatingFileHandler(
        log_filepath,
        maxBytes=getattr(config, "LOG_MAX_BYTES", 5 * 1024 * 1024),
        backupCount=getattr(config, "LOG_BACKUP_COUNT", 5),
...
def log_security_event(*, user_id: str | None, action: str, details: str) -> None:
    logger = security_audit_logger()
    uid = user_id or "unknown"
    logger.info("user=%s action=%s detail=%s", uid, action, details)
