(function () {
  ...
  const applyBaseToCard = (card, base, statusLabels) => {
    ...
    const liveToggle = card.querySelector('[data-card-live-toggle]');
    if (liveToggle) {
      const isLive = Boolean(base.is_live);
      liveToggle.checked = isLive;
    }

    const liveStateText = card.querySelector('[data-live-state-text]');
    if (liveStateText) {
      liveStateText.textContent = base.is_live ? 'Live' : 'Staged (not live)';
    }
  };
  ...
  const ready = () => {
    ...
    if (sanitizeBtn) {
      sanitizeBtn.addEventListener('click', async () => {
        ...
        openModal();
        try {
          const startRes = await postJSON('/admin/mockups/bases/sanitize-sync', {});
          if (!startRes || startRes.status !== 'ok') {
            throw new Error((startRes && startRes.message) || 'Sanitize & Sync failed');
          }

          const pollUntilDone = async () => {
            while (true) {
              const pollRes = await getJSON('/admin/mockups/bases/sanitize-sync/status');
              const detail = (pollRes && pollRes.detail) || {};
              const phase = String(detail.status || '').toLowerCase();
              ...
              if (phase === 'completed') {
                ...
                const discovered = Number(summary.discovered_orphans || 0);
                if (statusEl) {
                  statusEl.textContent = `Done. Renamed ${renamed}, thumbs ${thumbs}, coords ${coords}, discovered ${discovered}, removed empty dirs ${removed}.`;
                }
                window.setTimeout(() => {
              ...
              if (phase === 'failed') {
                throw new Error(String(detail.message || 'Sanitize & Sync failed'));
              }

              if (statusEl) {
                statusEl.textContent = String(detail.message || 'Running Sanitize & Sync…');
              }
    ...
    document.querySelectorAll('[data-mockup-card]').forEach((card) => {
      ...
      const liveApplyBtn = card.querySelector('[data-card-live-apply]');
      if (liveApplyBtn) {
        liveApplyBtn.addEventListener('click', async () => {
          const liveToggle = card.querySelector('[data-card-live-toggle]');
          const live = Boolean(liveToggle && liveToggle.checked);
          liveApplyBtn.disabled = true;
          try {
            const res = await postJSON(card.dataset.endpointPublication, { live });
            if (res && res.base) {
              handleCardUpdate(res.base);
            }
          } catch (err) {
            alert(err.message || 'Publication update failed');
          } finally {
            liveApplyBtn.disabled = false;
          }
        });
      }
      ...
      const liveToggle = card.querySelector('[data-card-live-toggle]');
      const liveStateText = card.querySelector('[data-live-state-text]');
      if (liveToggle && liveStateText) {
        liveToggle.addEventListener('change', () => {
          liveStateText.textContent = liveToggle.checked ? 'Live' : 'Staged (not live)';
        });
      }
    });
